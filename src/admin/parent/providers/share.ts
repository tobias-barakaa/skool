...
// Complete Parent Service with both existing and new methods
@Injectable()
...
export class EmailService {
  async sendParentInvitation(
    ...
    const invitationUrl = `${process.env.FRONTEND_URL}/accept-invitation?token=${token}&type=parent`;
    
    const studentList = studentNames.length > 0 
      ? `You have been invited to monitor the following students: ${studentNames.join(', ')}`
      : 'You have been invited to monitor your child(ren)';

    const subject = `Parent Invitation - ${schoolName}`;
    const html = `
      <h2>Parent Invitation</h2>
      <p>Dear ${parentName},</p>
      <p>You have been invited by ${invitedByName} to join ${schoolName} as a parent.</p>
      <p>${studentList}</p>
...
export class InvitationController {
  ...
  async acceptInvitation(@Param('token') token: string) {
    ...
    if (invitation.type === InvitationType.PARENT) {
      return await this.parentService.acceptParentInvitation(token);
    }

    throw new BadRequestException('Unknown invitation type');
  }
