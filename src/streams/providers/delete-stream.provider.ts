...
export class DeleteStreamProvider {
  ...
  async execute(id: string, user: ActiveUserData): Promise<boolean> {
    ...
    if (!user || !user.sub) {
      throw new ForbiddenException('User must be authenticated to delete streams');
    }
    ...
    try {
      ...
      throw new BadRequestException('Failed to delete stream. It may have associated data.');
    }
  }
